...
class TagAdminController extends Controller
{
    ...
    public function index($req, $res, $params)
    {
        ...
        $this->app->setTitle(AdminLang::t('tags', 'nav'));
        $tags = DB::getAll('SELECT * FROM tag ORDER BY slug ASC') ?: [];
        return $this->app->view->render('index', compact('tags'));
    }
    ...
    public function store($req, $res, $params)
    {
        ...
        header('Location: /' . $lang . '/admin/tag');
        exit;
    }
    ...
    public function edit($req, $res, $params)
    {
        $this->requireAdmin($res);
        $id  = (int)($params[0] ?? 0);
        $row = DB::findOne('tag', ' id = :id ', [':id' => $id]);
